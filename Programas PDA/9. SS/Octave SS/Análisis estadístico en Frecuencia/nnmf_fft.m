...
for m = 1:M-1
  if(do_not_delay == 1)
    X_t(m+1,:) = s1*amp_out1+s2*amp_out2;
  else
    X_t(m+1,:) = delay_f(s1,(m*d/c)*sin(doa1),N)+delay_f(s2,(m*d/c)*sin(doa2),N);
  end
end
...
while ((err > eps) && (its < maxIters))
  ...
  W = (X_f2_abs * o_f_abs') * pinv(o_f_abs * o_f_abs');
  W(W < 0) = 0;
  ...
  err = mean(sqrt(sum(abs(X_f2-X_est).^2)));
end
