...
%microphones. Esto es lo mismo que hacer el producto de matrices
x = s1 + s2;
y = real(ifft(fft(s1).*exp(-i*2*pi*w*(d/c)*sin(doas(1)*pi/180)) + fft(s2).*exp(-i*2*pi*w*(d/c)*sin(doas(2)*pi/180))));
z = real(ifft(fft(s1).*exp(-i*2*pi*w*(2*d/c)*sin(doas(1)*pi/180)) +fft(s2).*exp(-i*2*pi*w*(2*d/c)*sin(doas(2)*pi/180))));

ifft(fft(s1).*exp(-i*2*pi*w*(d/c)*sin(doas(1)*pi/180)))
ifft( fft(s2).*exp(-i*2*pi*w*(d/c)*sin(doas(2)*pi/180)))
