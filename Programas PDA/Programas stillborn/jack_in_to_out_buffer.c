...
int jack_callback(jack_nframes_t nframes, void *arg)
{
  ...
  for (i = 0; i < nframes; ++i)
  {
    ...
    // read
    out2[i] = buffer[buffer_i];

    // write
    buffer[buffer_i] = in1[i] + 3;
...
int main(int argc, char *argv[])
{
  ...
  // Si no hay un servidor levantado entonces devuelve error
  ...
  // Con el ampersand validamos si aplica cada uno de los posibles status
  ...
  input_port1 = jack_port_register(client, "input_1", JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
  input_port2 = jack_port_register(client, "input_2", JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
  ...
  // allocate memory of buffers BEFORe activating jack client
  ...
  if (jack_connect(client, serverports_names[0], jack_port_name(input_port2)))
  {
    printf("Cannot connect input port.\n");
    exit(1);
  }
